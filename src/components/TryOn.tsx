...
// // ---------- Watch tuning knobs ----------
// const WATCH_DEPTH = 0.9; // world distance for wrist unprojection (same ballpark as glasses)
// const WATCH_BASE_SCALE = 6; // fallback scale if hand span fails
// const WATCH_SCALE_K = 0.0007; // multiplier from hand span px -> scalar
// const WATCH_SCALE_MIN = 0.03; // min
// const WATCH_SCALE_MAX = 0.12; // max
// const WATCH_LOCAL_OFFSET = new THREE.Vector3(0.0, 0.0, 0.0); // local tweak if needed
...
//             // Initial size; will be overridden each frame
//             adjust.scale.setScalar(WATCH_BASE_SCALE);
//             adjust.position.copy(WATCH_LOCAL_OFFSET);
//             // A gentle default rotation (fine-tune if your watch GLB local axes need it)
...
//               else chosenIndex = 0; // fallback if no labels
//             }
...
//                 // Rotation: rotate in screen plane so the watch follows wrist twist
//                 // angle from wrist -> index base in screen space
//                 const ivx = indexBase.x * W - wx;
//                 const ivy = indexBase.y * H - wy;
//                 const angleZ = Math.atan2(ivy, ivx); // screen space angle
//                 // Build quaternion: rotate about camera's forward axis (Z in camera space)
//                 // Easiest: align to world Z with simple Euler
//                 watchAnchorRef.current.rotation.set(0, 0, angleZ);

//                 // Scale: from hand span (index-base to pinky-base)
...
//                 // Local nudge (if your model needs a small offset on the wrist)
...
//       initThree();
//       await Promise.all([loadGlasses(), loadHat(), loadWatch()]); // <<< NEW
//       renderThreeLoop();
...
//   // Visibility toggles propagate to groups + occluders (glasses/hat only)
//   useEffect(() => {
